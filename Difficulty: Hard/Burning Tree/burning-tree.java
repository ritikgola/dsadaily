...
class Solution
{
    ...
    // Function to calculate the time taken to burn the entire tree starting from the target node
    private static int burnTree(Node targetNode, Map<Node, Node> parentMap) {
        ...
        while (!queue.isEmpty()) {
            int size = queue.size();
            boolean burnedAtThisLevel = false;
            ...
            // Increase time only if any node was burned at this level
            if (burnedAtThisLevel) {
                time++;
            }
        }
        return time;
